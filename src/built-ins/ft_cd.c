...
/*   Updated: 2024/11/21 14:01:09 by artuda-s         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
cd -    -->        OLDPWD ?   cd "$OLDPWD" && pwd  : printf
("minihell: cd: OLDPWD not set,sir");

cd      -->        HOME ?   cd "$HOME" && pwd  : printf
("minihell: cd: HOME not set,sir");

cd --   -->     HOME ?  muda para $HOME  : printf("minihell:
 cd: HOME not set,sir");
...
void	ft_cd(t_exec *cmd)
{
	...
	else if (!cmd->av[1] || !ft_strcmp(cmd->av[1], "--"))
	{
		ft_cd_home(cmd);
		return ;
	}
	else if (cmd->av[1][0] == '\0')
		return ;
	else if (ft_strcmp(cmd->av[1], "-") == 0)
	{
		ft_cd_old_pwd(cmd);
		return ;
	}
