...
*WNOHANG*
  ...
  Esta opção faz com que waitpid() não bloqueie se o filho ainda não tiver terminado. Em vez disso, ele retorna imediatamente e você pode continuar executando outras tarefas.
*WUNTRACED*
...
pipe() #include <unistd.h>
    ...
    Funcionamento:
        ...
        TLDR
            pipe[0] => READING
            pipe[1] => WRITING
            Quando um processo e forked, o filho herda os mesmos fds entao podemos so
        fechar o lado do pipe que nao queremos usar. Usar um pipe por via de comunicacao.  
            Nao esquecer de fechar os fds que nao vamos usar no inicio e depois de os usar no fim
...
chdir() <unistd.h>
    ...
    Se você tentar mudar para um diretório que não existe ou para um diretório para o qual não tem permissões, chdir() falhará e errno será definido para indicar o erro (por exemplo, ENOENT para "No such file or directory" ou EACCES para "Permission denied").
...
opendir()   #include <sys/types.h>
    ...
    E preciso usar a closedir(DIR*)
Definitions
    DIR *dir:
        This is a pointer to a DIR structure, which represents a directory stream. You will use this pointer to access the directory entries.
    struct dirent *entry:
        This is a pointer to a struct dirent, which holds information about a directory entry, such as the name of the file or directory.
        struct dirent {
